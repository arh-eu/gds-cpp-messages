  ...
  public:
    ...
    private:
          ...
          , closed(false)
      {
          ...
          {
              mMask[c] = static_cast<unsigned char>(dist(rd));
          }
      }
      ...
      std::array<unsigned char, 4> mMask;
      size_t mMaskIndex;
    ...
    public:
      ...
      void send(const std::shared_ptr<OutMessage> &out_message,
                const std::function<void(const error_code &)> &callback = nullptr,
                unsigned char message_type = 129,
                size_t full_size = 0)
      {
        cancel_timeout();
        set_timeout();
        ...
        std::size_t length = (full_size != 0) ? full_size : out_message->size();

        //std::size_t max_additional_bytes = 14; // ws protocol adds at most 14 bytes
        std::shared_ptr<OutMessage> out_header_and_message = std::make_shared<OutMessage>(/*length + max_additional_bytes*/);

        out_header_and_message->put(static_cast<char>(message_type));
        ...
        for(std::size_t c = 0; c < 4; c++)
        {
          out_header_and_message->put(static_cast<char>(mMask[c]));
        }

        size_t buffer_original_size = out_message->size();
        for(mMaskIndex = 0; mMaskIndex < buffer_original_size; mMaskIndex++)
        {
          out_header_and_message->put(out_message->get() ^ mMask[mMaskIndex % 4]);
        }
      ...
      void send_data(const std::shared_ptr<OutMessage>& out_message,
                     const std::function<void(const error_code&)>& callback = nullptr)
      {
          cancel_timeout();
          set_timeout();

          std::shared_ptr<OutMessage> xor_message = std::make_shared<OutMessage>();

          size_t buffer_original_size = out_message->size();
          size_t c = 0;
          for(; c < buffer_original_size; c++)
          {
              xor_message->put(out_message->get() ^ mMask[(mMaskIndex + c) % 4]);
          }
          mMaskIndex += c;

          LockGuard lock(send_queue_mutex);
          send_queue.emplace_back(xor_message, callback);
          if(send_queue.size() == 1)
          {
              send_from_queue();
          }
    ...
    public:
      ...
      long timeout_request = 30;
      /// Idle timeout. Defaults to no timeout.
      long timeout_idle = 2 * 60;
    ...
    {
      {
        ...
        if(!io_service)
        {
            printf("[WebSocket] No IO service!\n");
            return;
        ...
        {
            printf("[WebSocket] IO service stopped, now launching!\n");
            restart(*io_service);}
*/
      }
      connect();
  ...
  protected:
    ...
    SocketClientBase(const std::string &host_port_path,
                     unsigned short default_port,
                     const std::shared_ptr<asio::io_service>& _io_service) noexcept
  ...
  public:
    SocketClient(const std::string &server_port_path, const std::shared_ptr<asio::io_service>& _io_service) noexcept
        : SocketClientBase<WS>::SocketClientBase(server_port_path, 80, _io_service)
    {}
